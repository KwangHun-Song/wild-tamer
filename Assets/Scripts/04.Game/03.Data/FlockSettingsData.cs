...
{
    [Header("정렬 가중치 — 이웃의 평균 이동 방향에 맞추려는 힘의 세기")]
    public float alignmentWeight = 1f;

    [Header("응집 가중치 — 이웃의 무게중심 방향으로 모이려는 힘의 세기")]
    public float cohesionWeight = 1f;

    [Header("분리 가중치 — 이웃과 최소 거리를 유지하려는 반발력의 세기")]
    public float separationWeight = 1.5f;

    [Header("추종 가중치 — 리더(플레이어)를 따라가려는 힘의 세기")]
    public float followWeight = 2f;

    [Header("회피 가중치 — 장애물을 피하려는 힘의 세기")]
    public float avoidanceWeight = 2f;

    [Header("이웃 인식 반경 — 이 거리 이내의 유닛을 이웃으로 간주한다")]
    public float neighborRadius = 3f;

    [Header("도착 반경 — 리더와 이 거리 이내이면 Follow 힘을 선형 감소시켜 자연스럽게 정지한다")]
    public float arrivalRadius = 1f;

    [Header("최소 분리 거리 — 유닛 간 유지해야 할 최소 거리. 이보다 가까우면 분리력이 발생한다")]
